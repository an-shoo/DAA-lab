...
bool prefers(int womenPreferences[N][N], int w, int m, int m1) {
    for (int i = 0; i < N; i++) {
        if (womenPreferences[w][i] == m1)
            return true;
...
void stableMarriage(int menPreferences[N][N], int womenPreferences[N][N]) {
    int womenPartner[N];
    bool menFree[N];
    ...
    while (freeCount > 0) {
        ...
        for (int i = 0; i < N && menFree[m]; i++) {
            int w = menPreferences[m][i]; // Woman w is the ith preference of man m
            ...
            if (womenPartner[w] == -1) {
                ...
                if (prefers(womenPreferences, w, m, m1) == false) {
...
int main() {
    ...
    int womenPreferences[N][N] = {
        {0, 1, 2}, // Preferences of Woman 0
        {1, 2, 0}, // Preferences of Woman 1
        {2, 0, 1}, // Preferences of Woman 2
    };
    
    stableMarriage(menPreferences, womenPreferences); // Find and print stable matches
    return 0;
}
